...
export async function getHabits(options = {}) {
  ...
  if (options.sortBy) {
    ...
    habits.sort((a, b) => {
      let timeA = Number.isFinite(a.timestamp) ? a.timestamp : Number.isFinite(new Date(a.createdAt).getTime()) ? new Date(a.createdAt).getTime() : 0;
      let timeB = Number.isFinite(b.timestamp) ? b.timestamp : Number.isFinite(new Date(b.createdAt).getTime()) ? new Date(b.createdAt).getTime() : 0;
      return timeA - timeB;
    })
  }
