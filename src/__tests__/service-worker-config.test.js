...
describe('Service Worker Configuration', () => {
  describe('NavigationRoute Configuration', () => {
    test('should have navigation fallback configured with absolute path for production', () => {
      ...
      expect(productionConfig.navigateFallback).toBe('/aurorae-haven/index.html')
      expect(productionConfig.hasAllowlist).toBe(true)
      expect(productionConfig.hasDenylist).toBe(true)
    })

    test('should have navigation fallback configured with relative path for offline', () => {
  ...
  describe('Integration: Service Worker + 404 Redirect', () => {
    ...
    test('should verify navigateFallback path resolution for production', () => {
      // Verify that the navigateFallback path is absolute for production
      // This is critical for the fix to work
      const productionBase = '/aurorae-haven/'
      const navigateFallback = `${productionBase}index.html`
      
      // The service worker will use this absolute path
      expect(navigateFallback).toBe('/aurorae-haven/index.html')
      expect(navigateFallback.startsWith('/')).toBe(true)
    })

    test('should verify navigateFallback path resolution for offline', () => {
      // Verify that the navigateFallback path is relative for offline
      const offlineBase = './'
      const navigateFallback = `${offlineBase}index.html`
      
      // The service worker will use this relative path
