...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let events = [];
  try {
    if (window.sjEvents && Array.isArray(window.sjEvents)) events = window.sjEvents;
  } catch {}
  if (!events.length) {
  ...
  if (!events.length) {
    events = [
      { id:'1', title:'Exemple', start:'2025-09-19T09:00:00', end:'2025-09-19T10:00:00',
        extendedProps:{ label:'Demo', description:'Description de démonstration multi-lignes.' } }
    ];
  }
