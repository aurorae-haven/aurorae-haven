...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (e.key === "Enter" && !e.shiftKey && !e.ctrlKey && !e.metaKey) {
      ...
      if (taskListMatch) {
        ...
        if (content.trim() === "") {
          ...
          const newValue = editor.value.substring(0, lineStart) + editor.value.substring(cursorPos);
          ...
          e.preventDefault();
          const newItem = `\n${indent}${marker} [ ] `;
          const newValue = editor.value.substring(0, cursorPos) + newItem + editor.value.substring(cursorPos);
          editor.value = newValue;
          editor.selectionStart = editor.selectionEnd = cursorPos + newItem.length;
          localStorage.setItem("brainDumpContent", editor.value);
          renderMarkdown();
        }
  ...
  function showVersionHistory() {
    ...
    content.style.cssText = 'max-width:600px;max-height:80vh;overflow-y:auto;padding:20px;';
    ...
    versions.forEach((version, index) => {
      ...
      item.style.cssText = 'padding:10px;margin:10px 0;border:1px solid #ccc;border-radius:4px;';
  ...
  function showDiff(oldContent, newContent) {
    ...
    const content = document.createElement('div');
    content.className = 'modal-content card';
    content.style.cssText = 'max-width:800px;max-height:80vh;overflow-y:auto;padding:20px;';
    ...
    diffView.style.cssText = 'font-family:monospace;font-size:12px;white-space:pre-wrap;';
    
    diff.forEach(item => {
      const line = document.createElement('div');
      if (item.type === 'added') {
        line.style.cssText = 'background:#d4edda;color:#155724;padding:2px 5px;';
        line.textContent = '+ ' + item.line;
      } else if (item.type === 'removed') {
        line.style.cssText = 'background:#f8d7da;color:#721c24;padding:2px 5px;';
        line.textContent = '- ' + item.line;
      } else {
  ...
  function showBacklinks() {
    ...
    if (links.length === 0) {
      ...
      empty.textContent = 'No backlinks found. Use [[link text]] syntax to create links.';
      ...
      links.forEach(link => {
        ...
        item.style.cssText = 'padding:10px;margin:5px 0;border:1px solid #ccc;border-radius:4px;';
  ...
  function attachFile() {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        const cursorPos = editor.selectionStart;
        editor.value = editor.value.substring(0, cursorPos) + fileRef + editor.value.substring(cursorPos);
        localStorage.setItem("brainDumpContent", editor.value);
        renderMarkdown();
        
        AccessibilityHelper.announceToScreenReader(`File ${file.name} attached`);
      } catch (err) {
