...
function validateServiceWorker() {
  ...
  if (!fs.existsSync(distDir)) {
    console.log('⚠️  Service worker not yet generated (run "npm run build" first)')
    warnings++
    return
  }
  
  if (!fs.existsSync(swPath)) {
    console.error('❌ Service worker file (sw.js) not found in dist/')
    errors++
    return
  }

  console.log('✅ Service worker file (sw.js) generated by vite-plugin-pwa')
  
  const content = fs.readFileSync(swPath, 'utf8')

  // Check for Workbox presence (generated by vite-plugin-pwa)
  if (content.includes('workbox') || content.includes('precache')) {
    console.log('✅ Service worker uses Workbox for caching')
  } else {
    console.error('❌ Service worker missing Workbox configuration')
    errors++
  }
}
...
function validateIndexHTML() {
  ...
  if (fs.existsSync(distIndexPath)) {
    ...
    if (distContent.includes('rel="manifest"')) {
      console.log('✅ Built index.html links to manifest (injected by vite-plugin-pwa)')
    } else {
      console.error('❌ Built index.html missing manifest link')
      errors++
    }
  } else {
    console.log('⚠️  Built index.html not found (run "npm run build" first)')
    warnings++
  }
}
...
function validateServiceWorkerRegistration() {
  ...
  if (content.includes('VitePWA') && content.includes('registerType')) {
    console.log('✅ Service worker registration configured (vite-plugin-pwa)')
  } else {
    console.error('❌ vite-plugin-pwa not configured in vite.config.js')
    errors++
  }
}
